...
bot.on("message", async message => {
    ...
    if(message.author.id=='217616223821758464'){dabcount++}
    else {return message.reply("Lul no");} 
    }
   
    if (message.content.startsWith('/kick'|'/KICK'|'/Kick' )) {
       ...
       if(!message.member.hasPermission("KICK_MEMBERS")) {return message.reply("Lul no");} 
       ...
       member.addRole('483977933615267852');
       member.setVoiceChannel('483251552744898580');
       const timer = Math.floor(Math.random()*(30000-15000+1)+15000);
       console.log(`Kick target timed for ${timer}`);
       setTimeout(function(){setTimeout(function(){member.removeRole('483977933615267852')},timer);},1000);
...
bot.login('NDgzMDIzNTI0MjE1MzkwMjE4.DmciHw.gcT69B9goyPuAgCSrqga7_5cdHI');